...
export default function PaymentsPage() {
  ...
  const handleLookup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Không tìm thấy giao dịch');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout
      eyebrow="Tra cứu tài chính"
      title="Thanh toán"
      subtitle="Tra cứu chi tiết giao dịch theo mã đơn hàng."
    ...
    >
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="rounded-[30px] border border-white/10 bg-[#0F172A]/80 p-6 shadow-2xl shadow-black/20 backdrop-blur-xl">
        <div className="mb-5"><p className="text-sm font-medium text-slate-400">Tra cứu thanh toán</p><h2 className="mt-1 text-xl font-bold tracking-tight text-white">Tìm giao dịch</h2></div>
        <form onSubmit={handleLookup} className="flex flex-col gap-3 sm:flex-row">
          <label className="relative flex-1">
            ...
            <input value={orderId} onChange={(event) => setOrderId(event.target.value)} required placeholder="Nhập mã đơn hàng thanh toán" className="h-11 w-full rounded-2xl border border-white/10 bg-white/[0.04] pl-10 pr-4 text-sm text-white outline-none transition placeholder:text-slate-500 focus:border-blue-400/60" />
          </label>
          <button type="submit" disabled={loading} className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 px-5 text-sm font-semibold text-white transition hover:from-blue-500 hover:to-purple-500 disabled:opacity-50"><Search className={cn('h-4 w-4', loading && 'animate-pulse')} />Tra cứu</button>
        </form>
        ...
        {payment && (
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {[
              { label: 'Mã đơn hàng', value: payment.orderId, icon: Hash },
              { label: 'Số tiền', value: formatCurrency(payment.amount), icon: Banknote },
              { label: 'Nhà cung cấp', value: payment.provider.toUpperCase(), icon: CreditCard },
              { label: 'Mã giao dịch', value: payment.vnpTransactionNo || '--', icon: Hash },
              { label: 'Mã ngân hàng', value: payment.vnpBankCode || '--', icon: CreditCard },
              { label: 'Thời gian thanh toán', value: formatDate(payment.paidAt), icon: Banknote },
            ].map((item) => (
            ...
            <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Trạng thái</p>
              <span className={cn('inline-flex rounded-full border px-3 py-1 text-xs font-semibold', statusClasses[payment.status])}>{statusLabels[payment.status]}</span>
            </article>
            <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 md:col-span-2">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Nội dung đơn hàng</p>
