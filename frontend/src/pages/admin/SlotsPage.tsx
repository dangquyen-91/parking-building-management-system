...
export default function SlotsPage() {
  ...
  const loadSlots = async (page = pagination.page) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Khong tai duoc danh sach slots');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : 'Khong tai duoc bo loc slots');
      }
      await loadSlots(1);
    };
    initialize();
  ...
  return (
    <AdminLayout
      eyebrow="Parking Capacity"
      title="Parking Slots Directory"
      subtitle="Theo doi trang thai cac vi tri do xe hoi trong he thong."
    ...
    >
      <div className="space-y-6">
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[
            { label: 'Total Slots', value: stats.total, icon: SquareParking, tone: 'text-blue-300' },
            { label: 'Empty This Page', value: stats.empty, icon: CheckCircle2, tone: 'text-emerald-300' },
            { label: 'Occupied This Page', value: stats.occupied, icon: Building2, tone: 'text-purple-300' },
            { label: 'Maintenance This Page', value: stats.attention, icon: AlertTriangle, tone: 'text-amber-300' },
          ].map((item, index) => (
        ...
        <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16, duration: 0.4 }} className="rounded-[30px] border border-white/10 bg-[#0F172A]/80 p-6 shadow-2xl shadow-black/20 backdrop-blur-xl">
          <div className="mb-5 flex flex-col gap-3">
            <div><p className="text-sm font-medium text-slate-400">Slot Directory</p><h2 className="mt-1 text-xl font-bold tracking-tight text-white">Danh sach vi tri do xe</h2></div>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-[1.3fr_1fr_1fr_auto]">
              ...
              <select value={floorId} onChange={(event) => setFloorId(event.target.value)} className="h-11 rounded-2xl border border-white/10 bg-[#111827] px-4 text-sm text-white outline-none focus:border-blue-400/60">
                <option value="">All car floors</option>
                {visibleFloors.map((floor) => <option key={floor.id} value={floor.id}>{floor.building.name} - Floor {floor.floorNumber}</option>)}
              </select>
              ...
              <button onClick={() => loadSlots(1)} disabled={loading} className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl border border-blue-400/25 bg-blue-400/10 px-5 text-sm font-semibold text-blue-200 transition hover:border-blue-300/50 disabled:opacity-50">
                {loading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}Filter
              </button>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[860px] text-left">
              ...
              <tbody className="divide-y divide-white/[0.06]">
                {loading ? (
                  <tr><td colSpan={6} className="py-12 text-center text-sm text-slate-400">Loading slots...</td></tr>
                ) : slots.length === 0 ? (
                  <tr><td colSpan={6} className="py-12 text-center text-sm text-slate-400">Khong co slot phu hop.</td></tr>
                  ...
                  <motion.tr key={slot.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04, duration: 0.28 }} className="transition hover:bg-white/[0.03]">
                    ...
                    <td className="py-4 text-sm text-slate-300"><span className="inline-flex items-center gap-2"><Layers3 className="h-4 w-4 text-slate-500" />Floor {slot.floor.floorNumber}</span></td>
                    <td className="py-4 text-sm capitalize text-slate-300">{slot.vehicleType}</td>
                    <td className="py-4"><span className={cn('inline-flex rounded-full border px-3 py-1 text-xs font-semibold capitalize', slotStatusClasses[slot.status])}>{slot.status}</span></td>
          ...
          <div className="mt-5 flex flex-col gap-3 border-t border-white/10 pt-5 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-slate-400">Showing page <span className="font-semibold text-white">{pagination.page}</span> of <span className="font-semibold text-white">{pagination.totalPages || 1}</span>, 5 slots per page</p>
