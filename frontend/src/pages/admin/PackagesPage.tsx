...
export default function PackagesPage() {
  ...
  const loadPackages = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Khong tai duoc danh sach packages');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout
      ...
      subtitle="Theo doi cac goi gui xe dang duoc cau hinh trong he thong."
    ...
    >
      <div className="space-y-6">
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[
            { label: 'Total Packages', value: stats.total, icon: Boxes, tone: 'text-blue-300' },
            { label: 'Active Packages', value: stats.active, icon: CheckCircle2, tone: 'text-emerald-300' },
            { label: 'Car Packages', value: stats.car, icon: Car, tone: 'text-purple-300' },
            { label: 'Motorcycle Packages', value: stats.motorcycle, icon: Bike, tone: 'text-amber-300' },
          ].map((item, index) => (
        ...
        <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16, duration: 0.4 }} className="rounded-[30px] border border-white/10 bg-[#0F172A]/80 p-6 shadow-2xl shadow-black/20 backdrop-blur-xl">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div><p className="text-sm font-medium text-slate-400">Package Directory</p><h2 className="mt-1 text-xl font-bold tracking-tight text-white">Danh sach goi gui xe</h2></div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] text-left">
              ...
              <tbody className="divide-y divide-white/[0.06]">
                {loading ? (
                  <tr><td colSpan={6} className="py-12 text-center text-sm text-slate-400">Loading packages...</td></tr>
                ) : packages.length === 0 ? (
                  <tr><td colSpan={6} className="py-12 text-center text-sm text-slate-400">Khong co package phu hop.</td></tr>
                  ...
                  <motion.tr key={pkg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04, duration: 0.28 }} className="transition hover:bg-white/[0.03]">
                    ...
                    <td className="py-4 text-sm capitalize text-slate-300">{pkg.vehicleType}</td>
                    <td className="py-4 text-sm font-semibold text-white">{pkg.durationDays} days</td>
                    ...
                    <td className="py-4"><span className={cn('inline-flex rounded-full border px-3 py-1 text-xs font-semibold', pkg.isActive ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-300' : 'border-red-400/20 bg-red-400/10 text-red-300')}>{pkg.isActive ? 'Active' : 'Inactive'}</span></td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
