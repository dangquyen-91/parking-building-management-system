...
function BookingContent() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fbff] pt-28 text-slate-950">
      <main className="container mx-auto px-6 pb-20 md:px-12">
        <BookingHero
          eyebrow="Đặt chỗ vãng lai"
          title="Giữ lịch gửi ô tô và thanh toán trước qua VNPay"
          description="Nhập biển số, chọn khung giờ gửi xe, hệ thống sẽ tạo booking theo tầng ô tô vãng lai phù hợp."
        />
        <section className="mx-auto mt-12 grid max-w-6xl gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
          <BookingForm
            ...
            hasActiveSubscriptions={booking.ownPlates.length > 0}
            isOutsideSubscription={!booking.ownPlates.includes(booking.plate)}
            plateValid={booking.plateValid}
          />
          <BookingSummary
            values={booking.values}
            plate={booking.plate}
            ...
            previewHours={booking.previewHours}
            previewAmount={booking.previewAmount}
            breakdown={booking.feeBreakdown}
