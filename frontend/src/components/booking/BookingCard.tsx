...
function getPendingRemainingMs(createdAt: string) {
  ...
  return Number.isFinite(createdAtMs)
    ? Math.max(0, createdAtMs + pendingTtlMs - Date.now())
    : null;
}
...
export function BookingCard({ booking, index, cancelling, onCancel }: Props) {
  ...
  const [pendingRemainingMs, setPendingRemainingMs] = useState<number | null>(
    () =>
      booking.status === "pending"
        ? getPendingRemainingMs(booking.createdAt)
        : null,
  );
  ...
  useEffect(() => {
    ...
    const updateCountdown = () =>
      setPendingRemainingMs(getPendingRemainingMs(booking.createdAt));
    updateCountdown();
    ...
    >
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Booking #{booking.id} · Tầng{" "}
              {booking.floor?.floorNumber ?? booking.floorId} ·{" "}
              {booking.prepaidHours} giờ trả trước
            </p>
          </div>
        </div>
        {canCancel && (
          <button
            ...
            disabled={cancelling}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 text-sm font-bold text-red-600 disabled:opacity-60"
          >
            {cancelling ? (
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        ...
        <Detail
          icon={ReceiptText}
          label="Số tiền"
          value={formatBookingCurrency(booking.amount)}
        />
      </div>
      <div className="mt-3">
        <Detail
