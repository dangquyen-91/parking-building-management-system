...
import { BOOKING_PRICE } from '../../hooks/useBookingForm';

interface Props {
  plate: string; customerEmail: string; durationHours: number; previewHours: number | null;
  previewAmount: number | null; submitError: string | null; submitting: boolean; ready: boolean; onSubmit: () => void;
}

export function BookingSummary({ plate, customerEmail, durationHours, previewHours, previewAmount, submitError, submitting, ready, onSubmit }: Props) {
  return (
    <aside className="lg:sticky lg:top-28 lg:self-start"><div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_38px_rgba(15,23,42,0.11)]">
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4">
          ...
          <div className="mt-4 flex items-end justify-between border-t border-slate-200 pt-4"><span className="text-sm font-semibold text-slate-500">Số tiền</span><span className="text-xl font-black text-blue-600">{formatBookingCurrency(previewAmount ?? BOOKING_PRICE)}</span></div>
