...
import { BOOKING_DURATION_OPTIONS, estimateCarFee, CAR_HOUR_PRICE, CAR_NIGHT_SURCHARGE, CAR_NIGHT_START, CAR_NIGHT_END, type BookingFieldErrors, type BookingFormValues } from '../../hooks/useBookingForm';
...
export function BookingForm({ values, onChange, isAuthenticated, hasActiveSubscriptions,
  isOutsideSubscription, plateValid, errors }: Props) {
  ...
  const inputBorder = (error?: string) => error ? 'border-red-300 focus:border-red-500' : 'border-slate-200 focus:border-blue-500';
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Biển số xe" wide>
          <input value={values.licensePlate} onChange={(e) => onChange('licensePlate', e.target.value.toUpperCase())} placeholder="VD: 51A-12345"
            aria-invalid={Boolean(errors.licensePlate)} className={cn(inputClass, 'text-base font-bold tracking-widest', inputBorder(errors.licensePlate))} />
          <FieldError message={errors.licensePlate} />
        </Field>
        <Field label="Tên khách hàng"><input maxLength={101} value={values.customerName} onChange={(e) => onChange('customerName', e.target.value)} placeholder="Tên người đặt" aria-invalid={Boolean(errors.customerName)} className={cn(inputClass, inputBorder(errors.customerName))} /><FieldError message={errors.customerName} /></Field>
        <Field label="Số điện thoại"><input inputMode="numeric" maxLength={15} value={values.customerPhone} onChange={(e) => onChange('customerPhone', e.target.value)} placeholder="VD: 0901234567" aria-invalid={Boolean(errors.customerPhone)} className={cn(inputClass, inputBorder(errors.customerPhone))} /><FieldError message={errors.customerPhone} /></Field>
        <Field label="Email nhận xác nhận booking" wide>
          <input type="email" value={values.customerEmail} onChange={(e) => onChange('customerEmail', e.target.value)} placeholder="VD: ban@example.com" aria-invalid={Boolean(errors.customerEmail)} className={cn(inputClass, inputBorder(errors.customerEmail))} />
        ...
        <Field label="Thời gian bắt đầu">
          <input type="datetime-local" value={values.startTime} onChange={(e) => onChange('startTime', e.target.value)} aria-invalid={Boolean(errors.startTime)} className={cn(inputClass, 'font-bold', inputBorder(errors.startTime))} />
        ...
        <Field label="Ghi chú" wide><textarea maxLength={501} value={values.note} onChange={(e) => onChange('note', e.target.value)} rows={4} placeholder="Thông tin thêm cho nhân viên bãi xe" aria-invalid={Boolean(errors.note)} className={cn('mt-2 w-full resize-none rounded-2xl border bg-slate-50 px-4 py-3 text-sm font-medium outline-none', inputBorder(errors.note))} /><div className="mt-1 flex justify-between"><FieldError message={errors.note} /><span className="ml-auto text-xs text-slate-400">{values.note.length}/500</span></div></Field>
      </div>
...
function FieldError({ message }: { message?: string }) {
  return message ? <p role="alert" className="mt-1.5 text-xs font-semibold text-red-600">{message}</p> : null;
}
